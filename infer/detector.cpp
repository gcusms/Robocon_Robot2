...
Detector::Detector(std::string &xml_path) {
    _xml_path = xml_path;
}
...
//初始化
bool Detector::init(double cof_threshold,double nms_area_threshold){
...
bool Detector::process_frame(Mat& inframe,vector<Object>& detected_objects){
    ...
    for(int i=0;i<final_id.size();++i){
        ...
        detected_objects.push_back(Object{
            ...
            names2[label[final_id[i]]],resize_rect,label[final_id[i]]
        });
